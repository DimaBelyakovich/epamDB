...
MERGE INTO EVENT
    ...
    EVENT,
    EVENT_DATE,
    DRIVER.NAME POLE_POSITION
    FROM DATA_TABLE DT1
    LEFT JOIN DRIVER
    ON DRIVER.NAME = DT1.POLE_POSITION
    ) T2
    ON (EVENT.NAME = T2.EVENT)
WHEN NOT MATCHED THEN
    INSERT (NAME, EVENT_DATE, POLE_POSITION)
    VALUES(T2.EVENT, T2.EVENT_DATE, T2.POLE_POSITION);
--OK -5
...
WHEN NOT MATCHED THEN
    INSERT(SEASON_EVENT.SEASON_ID, SEASON_EVENT.EVENT_ID, SEASON_EVENT.TRACK_ID, SEASON_EVENT.WINNER)
    VALUES(T1.S_ID, T1.E_ID, T1.T_ID, T1.D_ID);
--OK -6
